<form wire:submit.prevent="confirm">
    <div x-cloak x-data="{ isOpen: false }" x-on:open-subscription-edit-modal.window="isOpen = true" x-on:close-subscription-edit-modal.window="isOpen = false" role="dialog" aria-modal="true" class="block" aria-label="confirmation modal">
                    ...
                    <div class="space-y-2">
                        <div class="p-4 space-y-4 text-center">
                            ...
                            <div>
                                ...
                                <div class="mt-2 flex items-center space-x-1 group">
                                    <div class="flex-1">
                                        <input type="password" wire:model.defer="refcode" autocomplete="off" id="refcode" required class="block w-full h-10 duration-75 border-gray-300 rounded-lg shadow-sm focus:border-blue-600 focus:ring-1 focus:ring-inset focus:ring-blue-600 focus:ring-opacity-30" />
                                    </div>
                                </div>
                                @error('refcode') <p class="text-xs text-red-500 error">{{ $message }}</p> @enderror
                            </div>
